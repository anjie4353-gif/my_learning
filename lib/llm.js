// Server-only LLM helper using Emergent Universal LLM Key
// OpenAI-compatible gateway

const EMERGENT_BASE = 'https://integrations.emergentagent.com/llm/v1'

export async function callLLM({ system, messages, model = 'gpt-5', json = false, temperature = 0.7, max_tokens }) {
  const allMessages = []
  if (system) allMessages.push({ role: 'system', content: system })
  for (const m of messages) allMessages.push(m)

  const body = { model, messages: allMessages, temperature }
